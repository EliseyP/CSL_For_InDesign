...
//--------------------------------------------------->Прогресс бар
    ...
    //                                                                                                     0   1    2    3
        ...
        var NBSP = " "; // Неразрывный пробел
        var SPACE = " "; // Обычный пробел
        var first_line = myParentTextFrames[i+1].lines[0];
        var first_line_str = first_line.contents;
		
        // Удалять все кроме двоеточия и дефиса после границы первого слова.
        // Общий случай - для строки с обычными пробелами, 
        // без неразрывного пробела.        
        var findWhat = "\\>[^-:]";
        var changeTo = "";
        
        /*
            Два случая:
            1. В первой строке есть пробелы. 
                Это а) либо как минимум одно слово через пробел с другими, 
                или б) два слова через неразр-й пробел и далее через обычный пробел другие..
             2. В первой строке нет пробелов.
                Это либо а) одно слово во всей строке,
                либо б) только два слова через неразр-й пробел.
            */

        // В первой строке есть пробелы. 
        if (first_line_str.indexOf(SPACE) != -1){
            var Temp_list = first_line_str.split(" ");
            myFrame.texts[0].contents = Temp_list[0];
            
            // Если был неразрывный пробел.
            if (Temp_list[0].indexOf(NBSP) != -1){
                // Правила regex
                findWhat = "^(\\w+[[:punct:]]?\\s\\w+)\\>[^-:]";
                changeTo = "$1";
            }
        
            Temp_list = [];
        }
        // В первой строке нет пробелов.
        else{
            // NOTE: В words[i] попадают и знаки пунктуации.
            // (они позже удаляются grep'ом)
            var sFirstWord = first_line.words[0].contents;
            
            // Обычный вариант: первое слово - в кустоду.    
            if (first_line_str.indexOf(NBSP) == -1){
                myFrame.texts[0].contents = sFirstWord;
                sFirstWord = "";
            }
            else{ // Если был неразрывный пробел.
                if (first_line.words.length > 1){
                    var sSecondWord = first_line.words[1].contents;
                    if (sSecondWord.length > 0){
                        myFrame.texts[0].contents = sFirstWord + NBSP + sSecondWord;
                        sSecondWord = "";
                        
                        // Удалять все кроме двоеточия и дефиса после границы второго слова.
                        // Если после первого слова стоит запятая или иной знак пунктуации,
                        // то его также нужно оставить.
                        findWhat = "^(\\w+[[:punct:]]?\\s\\w+)\\>[^-:]";
                        changeTo = "$1";
                    }
                }
